...
Public Class frm_GRDAcceptBag

    Private Sub frm_GRDAcceptBag_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub flxParcel_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles flxParcel.CellClick
        ...
        If flxParcel.Rows.Count > 0 Then
            ...
            rsComSql.Open("SELECT ParNo, PktNo, IssPcsT + IssPcsB AS IssPcs, IssCts " & _
                          "FROM dbo.tblBAGIssues " & _
                          "WHERE ParNo = '" & selected_parno & "' AND GrdTrf = 1 AND Sec = 4 " & _
    ...
    Private Sub SaveAccept()
        ...
        For u = 0 To flxSelected.Rows.Count - 1
            AdoCN.Execute("UPDATE tblBAGIssues SET GrdTrf = 2,AcceptBy = '" & PBUser_EmpNo & "' WHERE ParNo = '" & flxSelected.Item(0, u).Value & "' AND PktNo = '" & flxSelected.Item(1, u).Value & "' AND GrdTrf = 1 AND Sec = 4")
        Next

        MsgBox("Packets Accepted Successfully", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, Me.Text)
        ClearFields()
        Load_Parcels()
    End Sub
