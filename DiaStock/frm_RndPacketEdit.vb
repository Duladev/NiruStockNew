...
Public Class frm_RndPacketEdit
    Dim ParcelNo As String
    Dim PacketNo As String

    Private Sub frm_RndPacketEdit_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub cmdParPkt_Click(sender As Object, e As EventArgs) Handles cmdParPkt.Click
        ...
        If Datavalid = True Then
            ...
            rsComSql.Open("SELECT ParNo,Grp,PktPcs,PktCts,Diameter FROM tblRndPacket WHERE ParNo = '" & ParcelNo & "' AND PktNo = '" & PacketNo & "' AND DelDate IS NOT NULL AND AccDate IS NOT NULL", AdoCN, 1, 1)
            If rsComSql.RecordCount Then
                strGroup = rsComSql.Fields("Grp").Value
                dblIssPcs = rsComSql.Fields("PktPcs").Value
                dblPktCts = rsComSql.Fields("PktCts").Value
                strDiameter = rsComSql.Fields("Diameter").Value
                blnFound = True
            Else
                blnFound = False
            End If
            ...
            If blnFound = True Then
                rsComSql = New ADODB.Recordset
                rsComSql.Open("SELECT PktNo FROM tblRndReturns WHERE ParNo = '" & ParcelNo & "' AND PktNo = '" & PacketNo & "' AND Sec = 14", AdoCN, 1, 1)
                If rsComSql.RecordCount Then
                Else
                    blnFound = False
                End If
                rsComSql = Nothing
                If blnFound = False Then MsgBox("Invalid Prep Finish", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, Me.Text) : cmdParPkt.Focus() : Exit Sub
            End If
    ...
    Private Sub Save()
        ...
        For intRow = 0 To flxDetails.Rows.Count - 1
            AdoCN.Execute("UPDATE tblRndPacket SET Diameter = '" & flxDetails.Item(5, intRow).Value & "' WHERE ParNo = '" & flxDetails.Item(0, intRow).Value & "' AND PktNo = '" & flxDetails.Item(1, intRow).Value & "'")
        Next
