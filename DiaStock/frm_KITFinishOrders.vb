...
Public Class frm_KITFinishOrders
    ...
    Private Sub Load_PolishedReturns()
        ...
        If txtOrder.Text <> "" Then
            strSelect = "SELECT TOP (100) PERCENT dbo.tblMixReturns.ParNo, dbo.tblMixPacket.PktRefNo, dbo.tblMixPacket.Pktside, dbo.tblMixPacket.AParNo, dbo.tblMixPacket.AssortNo, " & _
                            ...
                            "SUM(dbo.tblMixReturns.ExtPcs) AS Ext, dbo.tblMixPacket.Grp, dbo.tblKITOrders.Subject, dbo.tblKITOrdersDtls.CutChg, dbo.tblKITOrdersDtls.NLineNo, " & _
                            "dbo.tblKITOrdersDtls.GrCount, SUM(dbo.tblMixReturns.GrPcs) AS GrPcs, MAX(dbo.tblMixReturns.RetDate) AS RetDate " & _
                      "FROM dbo.tblMixPacket INNER JOIN dbo.tblMixReturns ON dbo.tblMixPacket.PktOrdNo = dbo.tblMixReturns.ParNo AND dbo.tblMixPacket.PktNo = dbo.tblMixReturns.PktNo INNER JOIN " & _
                            "dbo.tblKITOrders ON dbo.tblMixPacket.PktOrdNo = dbo.tblKITOrders.OrderNo INNER JOIN " & _
                            "dbo.tblKITOrdersDtls ON dbo.tblKITOrders.OrderNo = dbo.tblKITOrdersDtls.OrderNo AND dbo.tblMixPacket.PktRefNo = dbo.tblKITOrdersDtls.RefNo AND  " & _
                            "dbo.tblMixPacket.Pktside = dbo.tblKITOrdersDtls.Side INNER JOIN " & _
            ...
            If cmbRef.Text <> "" Then
                strWhere = strWhere & " AND dbo.tblMixPacket.PktRefNo = '" & cmbRef.Text & "' "
            End If
            ...
            strOrder = "GROUP BY dbo.tblMixReturns.ParNo, dbo.tblMixReturns.PktNo, dbo.tblMixPacket.PktRefNo, dbo.tblMixPacket.AParNo, dbo.tblMixPacket.PktCts, dbo.tblMixPacket.PktPcs, " & _
                            "dbo.tblMixPacket.AssortNo, dbo.tblMixReturns.Status, dbo.tblMixPacket.Pktside, dbo.tblMixPacket.Grp, dbo.tblKITOrders.Subject, dbo.tblKITOrdersDtls.CutChg, " & _
                            "dbo.tblAssortList.MarketPrice, dbo.tblKITOrdersDtls.NLineNo, dbo.tblKITOrdersDtls.GrCount " & _
        ...
        If Rs.RecordCount Then
            ...
            While Not Rs.EOF
                ...
                rsComSql.Open("SELECT SUM(LostPcs) AS LostPcs FROM tblMixReturns WHERE ParNo = '" & Rs.Fields("ParNo").Value & "' AND PktNo = '" & Rs.Fields("PktNo").Value & "' AND LostStatus = 0", AdoCN, 1, 1)
                ...
                intGrPcs = 0
                rsComSql = New ADODB.Recordset
                rsComSql.Open("SELECT SUM(GrPcs) AS GrPcs FROM tblMixReturns WHERE ParNo = '" & Rs.Fields("ParNo").Value & "' AND PktNo = '" & Rs.Fields("PktNo").Value & "' AND Sec = 15", AdoCN, 1, 1)
                If rsComSql.RecordCount Then
                ...
                vIssCts = 0
                rsComSql = New ADODB.Recordset
                mStrSQL = "SELECT Sum(RejPcs) As RejPcs, Sum(RejCts) As RejCts, Sum(RghCts) As RghCts, Sum(ExtPcs) As ExtPcs " & _
                          "FROM tblMixReturns " & _
                          "WHERE ParNo = '" & Rs.Fields("ParNo").Value & "' AND PktNo = '" & Rs.Fields("PktNo").Value & "' AND RejPcs + ExtPcs > 0"
                ...
                If rsComSql.RecordCount Then
                    If Not IsDBNull(rsComSql.Fields("RejPCs").Value) Then
                        ...
                        mRejRgh = Format(rsComSql.Fields("RghCts").Value, "#0.000")
                ...
                flxDetails.Rows.Add(Rs.Fields("ParNo").Value, strSubject, Rs.Fields("PktRefNo").Value, Rs.Fields("Pktside").Value,
                                    ...
                                    Rs.Fields("GrCount").Value, intGrPcs, Rs.Fields("NLineNo").Value, vIssuePcs, Format(Rs.Fields("RetDate").Value, "yyyy/MM/dd"))
    ...
    Private Sub Load_SavedData()
        ...
        rsComSql = New ADODB.Recordset
        If txtOrder.Text <> "" Then
            strSelect = "SELECT * FROM tblMixFinishOrders "
            ...
            mStrSQL = strSelect & strWhere & strOrder
        Else
            mStrSQL = "SELECT * FROM tblMixFinishOrders WHERE Status = '" & strType & "' ORDER BY OrderNo, Reference, PacketNo"
        End If
        rsComSql.Open(mStrSQL, AdoCN, 1, 1)
        If rsComSql.RecordCount Then
            rsComSql.MoveFirst()
            While Not rsComSql.EOF
                ...
                rsComSql_1.Open("SELECT Grp FROM tblMixPacket WHERE PktOrdNo = '" & rsComSql.Fields("OrderNo").Value & "' AND PktNo = '" & rsComSql.Fields("PacketNo").Value & "'", AdoCN, 1, 1)
    ...
    Private Sub SaveData()
        ...
        For iRow = 0 To flxDetails.Rows.Count - 1
            If flxDetails.Item(26, iRow).Value <> "" Then
                rsComSql = New ADODB.Recordset
                rsComSql.Open("SELECT NLineNo FROM tblKITOrdersDtls WHERE NLineNo = '" & flxDetails.Item(26, iRow).Value & "'", AdoCN, 1, 1)
        ...
        If optNew.Checked = True Then
            ...
            rsComSql = New ADODB.Recordset
            mStrSQL = "SELECT MAX(RecordNo) AS RecordNo FROM tblMixFinishOrders"
            rsComSql.Open(mStrSQL, AdoCN, 1, 1)
            If rsComSql.RecordCount Then
                If IsDBNull(rsComSql.Fields("RecordNo").Value) Then
                    vRecordNo = 1
                Else
            ...
            For iRow = 0 To flxDetails.RowCount - 1
                If flxDetails.Item(14, iRow).Value = True Or flxDetails.Item(14, iRow).Value = 1 Then
                    mStrSQL = "INSERT INTO tblMixFinishOrders(OrderNo,Subject,Reference,Side,ParNo,Assortment,AssPrice,PacketNo,FinishedPcs,FinishedCts,PacketPcs,PacketCts," & _
                    ...
                    If chkReject.Checked = False Then
                        AdoCN.Execute("UPDATE tblMixReturns SET Status = 2 WHERE ParNo = '" & flxDetails.Item(0, iRow).Value & "' AND PktNo = '" & flxDetails.Item(7, iRow).Value & "' AND  Sec = 18 AND Status  = 0")
                    Else
                        AdoCN.Execute("UPDATE tblMixReturns SET Status = 1 WHERE ParNo = '" & flxDetails.Item(0, iRow).Value & "' AND PktNo = '" & flxDetails.Item(7, iRow).Value & "' AND Status = 0")
                    End If
        ...
        Else
            For iRow = 0 To flxDetails.RowCount - 1
                If flxDetails.Item(14, iRow).Value = True Or flxDetails.Item(14, iRow).Value = 1 Then
                    mStrSQL = "UPDATE tblMixFinishOrders SET PacketPcs = " & CDbl(flxDetails.Item(10, iRow).Value) & ",PacketCts = " & CDbl(flxDetails.Item(11, iRow).Value) & ",FinishedPcs = " & CDbl(flxDetails.Item(8, iRow).Value) & ",FinishedCts = " & CDbl(flxDetails.Item(9, iRow).Value) & "," & _
    ...
    Private Sub Load_RejectPackets()
        ...
        If chkAll.Checked = True Then
            mStrSQL = "SELECT TOP (100) PERCENT dbo.tblMixReturns.ParNo, dbo.tblMixReturns.PktNo, dbo.tblMixPacket.PktPcs, " & _
                        ...
                        "ROUND(SUM(dbo.tblMixReturns.RejCts),3) AS RejCts, ROUND(dbo.tblMixPacket.PktCts - SUM(dbo.tblMixReturns.RejCts), 3) AS DifCts, dbo.tblMixPacket.AssortNo, dbo.tblKITOrders.Subject," & _
                      ...
                      "FROM dbo.tblMixPacket INNER JOIN dbo.tblMixReturns ON dbo.tblMixPacket.PktOrdNo = dbo.tblMixReturns.ParNo AND dbo.tblMixPacket.PktNo = dbo.tblMixReturns.PktNo INNER JOIN " & _
                        "dbo.tblKITOrders ON dbo.tblMixPacket.PktOrdNo = dbo.tblKITOrders.OrderNo INNER JOIN dbo.tblAssortList ON dbo.tblMixPacket.AssortNo = dbo.tblAssortList.Assortment " & _
                      "WHERE (dbo.tblMixReturns.Status = 0) " & _
                      "GROUP BY dbo.tblMixReturns.ParNo, dbo.tblMixReturns.PktNo, dbo.tblMixPacket.PktPcs, dbo.tblMixPacket.PktCts, dbo.tblMixPacket.AssortNo, dbo.tblKITOrders.Subject, " & _
                        "dbo.tblMixPacket.PktRefNo , dbo.tblMixPacket.Pktside, dbo.tblMixPacket.AParNo, dbo.tblAssortList.MarketPrice, dbo.tblMixPacket.Grp " & _
        ...
        Else
            mStrSQL = "SELECT TOP (100) PERCENT dbo.tblMixReturns.ParNo, dbo.tblMixReturns.PktNo, dbo.tblMixPacket.PktPcs, " & _
                        ...
                        "dbo.tblMixPacket.PktRefNo , dbo.tblMixPacket.Pktside, dbo.tblMixPacket.AParNo, dbo.tblAssortList.MarketPrice, dbo.tblMixPacket.Grp " & _
                      "FROM dbo.tblMixPacket INNER JOIN dbo.tblMixReturns ON dbo.tblMixPacket.PktOrdNo = dbo.tblMixReturns.ParNo AND dbo.tblMixPacket.PktNo = dbo.tblMixReturns.PktNo INNER JOIN " & _
                        "dbo.tblKITOrders ON dbo.tblMixPacket.PktOrdNo = dbo.tblKITOrders.OrderNo INNER JOIN dbo.tblAssortList ON dbo.tblMixPacket.AssortNo = dbo.tblAssortList.Assortment INNER JOIN " & _
        ...
        If rsComSql.RecordCount Then
            ...
            While Not rsComSql.EOF
                ...
                rsComSql_1 = New ADODB.Recordset
                rsComSql_1.Open("SELECT CutChg, NLineNo FROM tblKITOrdersDtls WHERE OrderNo = '" & rsComSql.Fields("ParNo").Value & "' AND RefNo = '" & rsComSql.Fields("PktRefNo").Value & "' AND Side = '" & rsComSql.Fields("Pktside").Value & "'", AdoCN, 1, 1)
                If rsComSql_1.RecordCount Then
    ...
    Private Sub Load_LabPackets()
        ...
        mStrSQL = "SELECT TOP (100) PERCENT dbo.tblMixReturns.ParNo, dbo.tblMixReturns.PktNo, dbo.tblMixReturns.Sec, dbo.tblMixReturns.EmpNo, dbo.tblMixReturns.RejPcs, dbo.tblMixReturns.RejCts, " & _
                        ...
                        "dbo.tblMixPacket.Pktside, dbo.tblKITOrders.Subject, dbo.tblKITOrders.Niruref, dbo.tblAssortList.CurrentCost, dbo.tblAssortList.MarketPrice, dbo.tblKITOrdersDtls.MaxCost, dbo.tblKITOrdersDtls.MaxType, " & _
                        "dbo.tblMixPacket.PktPcs, dbo.tblMixPacket.PktCts, dbo.tblKITOrdersDtls.NLineNo, dbo.tblKITOrdersDtls.GrCount, dbo.tblKITOrdersDtls.CutChg " & _
                      "FROM dbo.tblMixReturns INNER JOIN dbo.tblMixPacket ON dbo.tblMixReturns.ParNo = dbo.tblMixPacket.PktOrdNo AND dbo.tblMixReturns.PktNo = dbo.tblMixPacket.PktNo INNER JOIN " & _
                        "dbo.tblKITOrders ON dbo.tblMixPacket.PktOrdNo = dbo.tblKITOrders.OrderNo INNER JOIN dbo.tblAssortList ON dbo.tblMixPacket.AssortNo = dbo.tblAssortList.Assortment INNER JOIN " & _
                        "dbo.tblKITOrdersDtls ON dbo.tblKITOrders.OrderNo = dbo.tblKITOrdersDtls.OrderNo AND dbo.tblMixPacket.PktRefNo = dbo.tblKITOrdersDtls.RefNo AND dbo.tblMixPacket.Pktside = dbo.tblKITOrdersDtls.Side " & _
                      "WHERE (dbo.tblMixReturns.RejStatus = 1) AND (dbo.tblMixReturns.RejPcs > 0) AND (dbo.tblMixReturns.RejReason = N'Lab Checking Reject') AND (dbo.tblMixReturns.Sec = 16) " & _
    ...
    Private Sub txtOrder_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtOrder.KeyPress
        ...
        If Asc(e.KeyChar) = 13 Then
            ...
            rsComSql_1 = New ADODB.Recordset
            rsComSql_1.Open("SELECT RefNo FROM tblKITOrdersDtls WHERE OrderNo = '" & CInt(txtOrder.Text) & "' GROUP BY RefNo ORDER BY RefNo", AdoCN, 1, 1)
            If rsComSql_1.RecordCount Then
    ...
    Private Sub cmbRef_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbRef.SelectedIndexChanged
        cmbSide.Items.Clear()
        rsComSql_1 = New ADODB.Recordset
        rsComSql_1.Open("SELECT Side FROM tblKITOrdersDtls WHERE OrderNo = '" & CInt(txtOrder.Text) & "' AND RefNo = '" & cmbRef.Text & "' GROUP BY Side ORDER BY Side", AdoCN, 1, 1)
        If rsComSql_1.RecordCount Then
            rsComSql_1.MoveFirst()
            While Not rsComSql_1.EOF
    ...
    Private Sub frm_KITFinishOrders_Load(sender As Object, e As EventArgs) Handles MyBase.Load
