...
Public Class frm_GRDRghIssues
    Private Sub Load_ExportParcels()
        ...
        rsComSql.Open("SELECT DISTINCT TOP (100) PERCENT dbo.tblGrading_SizingTypes.Department, dbo.tblGrading_SizingTypes.ParNo " & _
                      ...
                      "WHERE (dbo.tblGrading_Parcel.Complete = 0) AND (dbo.tblGrading_SizingTypes.Ok = 0) AND " & _
                        "(dbo.tblGrading_SizingTypes.Department = 'Baguettes' OR dbo.tblGrading_SizingTypes.Department = 'Princess' OR " & _
                        "dbo.tblGrading_SizingTypes.Department = 'GradingPCU_N' OR dbo.tblGrading_SizingTypes.Department = 'GradingPCU' OR " & _
                        "dbo.tblGrading_SizingTypes.Department = 'Mix' OR dbo.tblGrading_SizingTypes.Department = 'Direct Import' OR " & _
                        "dbo.tblGrading_SizingTypes.Department = 'Princess2' OR dbo.tblGrading_SizingTypes.Department = 'Emerald' OR " & _
                        "dbo.tblGrading_SizingTypes.Department = 'Opening' OR dbo.tblGrading_SizingTypes.Department = 'GradingMix' OR " & _
                        "dbo.tblGrading_SizingTypes.Department = 'Davinci' OR dbo.tblGrading_SizingTypes.Department = 'Baguettes2' OR dbo.tblGrading_SizingTypes.Department = 'Carrer' OR " & _
                        "dbo.tblGrading_SizingTypes.Department = 'Emerald2' OR dbo.tblGrading_SizingTypes.Department = 'Emerald3' OR dbo.tblGrading_SizingTypes.Department = 'Rounds' OR " & _
                        "dbo.tblGrading_SizingTypes.Department = 'Asscher' OR dbo.tblGrading_SizingTypes.Department = 'Radiant') " & _
                      "ORDER BY dbo.tblGrading_SizingTypes.Department, dbo.tblGrading_SizingTypes.ParNo", AdoCN, 1, 1)
    ...
    Private Sub Load_ExportDetails(ByVal strParcel As String, ByVal strDepartment As String)
        ...
        rsComSql.Open("SELECT TOP (100) PERCENT dbo.tblGrading_SizingTypes.Department, dbo.tblGrading_SizingTypes.ReturnType, dbo.tblGrading_SizingTypes.ParNo,dbo.tblGrading_SizingTypes.PktNo, " & _
                            "SUM(dbo.tblGrading_SizingTypes.Pcs) AS Pcs, ROUND(SUM(dbo.tblGrading_SizingTypes.Cts), 3) AS Cts " & _
                      ...
                      "GROUP BY dbo.tblGrading_SizingTypes.Department, dbo.tblGrading_SizingTypes.ReturnType, dbo.tblGrading_SizingTypes.ParNo,dbo.tblGrading_SizingTypes.PktNo " & _
                      "ORDER BY dbo.tblGrading_SizingTypes.ReturnType", AdoCN, 1, 1)
        If rsComSql.RecordCount Then
            rsComSql.MoveFirst()
            While Not rsComSql.EOF
                intRghIssPcs = 0
                ...
                If rsComSql.Fields("Pcs").Value - intRghIssPcs > 0 Then
                    flxDetails.Rows.Add(rsComSql.Fields("ReturnType").Value,
                                        ...
                                        rsComSql.Fields("Cts").Value - dblRghIssCts,
                                        "0",
                                        "0",
    ...
    Private Sub frm_GRDRghIssues_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub Save()
        ...
        For intRow = 0 To flxDetails.Rows.Count - 1
            ...
            If CDbl(flxDetails.Item(4, intRow).Value) > CDbl(flxDetails.Item(2, intRow).Value) Then
                MsgBox(flxDetails.Item(0, intRow).Value & " Invalid Issue Pcs2", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, Me.Text)
                Exit Sub
            End If

            If Math.Round(CDbl(flxDetails.Item(5, intRow).Value), 3) > Math.Round(CDbl(flxDetails.Item(3, intRow).Value), 3) Then
                MsgBox(flxDetails.Item(0, intRow).Value & " Invalid Issue Cts2", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, Me.Text)
                Exit Sub
            End If

            If CDbl(flxDetails.Item(2, intRow).Value) = CDbl(flxDetails.Item(4, intRow).Value) Then
                If Math.Round(CDbl(flxDetails.Item(3, intRow).Value), 3) <> Math.Round(CDbl(flxDetails.Item(5, intRow).Value), 3) Then
                    MsgBox(flxDetails.Item(0, intRow).Value & " Invalid Issue Cts3", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, Me.Text)
                    Exit Sub
                End If
            End If
            ...
            If CDbl(flxDetails.Item(3, intRow).Value) = CDbl(flxDetails.Item(5, intRow).Value) Then
                If CDbl(flxDetails.Item(2, intRow).Value) <> CDbl(flxDetails.Item(4, intRow).Value) Then
                    MsgBox(flxDetails.Item(0, intRow).Value & " Invalid Issue Pcs3", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, Me.Text)
                    Exit Sub
                End If
            End If
        Next
            ...
            For intRow = 0 To flxDetails.Rows.Count - 1
                If CDbl(flxDetails.Item(4, intRow).Value) > 0 Then
                    AdoCN.Execute("INSERT INTO tblGrading_RghIssues(Department,ParNo,Assortment,IssPcs,IssCts,IssDate,IssTime,Type,PktNo) " & _
                                  "VALUES('" & flxDetails.Item(6, intRow).Value & "','" & flxDetails.Item(1, intRow).Value & "','" & flxDetails.Item(0, intRow).Value & "'," & CInt(flxDetails.Item(4, intRow).Value) & "," & _
                                        "" & CDbl(flxDetails.Item(5, intRow).Value) & ",'" & Format(Date.Now, "MM/dd/yyyy") & "','" & Format(Date.Now, "HH:mm:ss") & "','" & strType & "','" & flxDetails.Item(7, intRow).Value & "')")
                End If
            Next
