...
Public Class frm_GRDSizingIssue
    ...
    Private Sub frm_GRDSizingIssue_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub Load_Details()
        ...
        dblIssCts = 0
        rsComSql = New ADODB.Recordset
        rsComSql.Open("SELECT TOP (100) PERCENT dbo.tblGrading_SizingPacket.Department, dbo.tblGrading_SizingPacket.ParNo, dbo.tblGrading_SizingPacket.PktNo, " & _
                        "dbo.tblGrading_SizingPacket.PktPcs, dbo.tblGrading_SizingPacket.PktCts " & _
                      "FROM dbo.tblParcel INNER JOIN dbo.tblGrading_SizingPacket ON dbo.tblParcel.Depart = dbo.tblGrading_SizingPacket.Department AND  " & _
                        "dbo.tblParcel.GrpParNo = dbo.tblGrading_SizingPacket.ParNo LEFT OUTER JOIN " & _
                        "dbo.tblGrading_SizingIssues ON dbo.tblGrading_SizingPacket.Department = dbo.tblGrading_SizingIssues.Department AND " & _
                        "dbo.tblGrading_SizingPacket.ParNo = dbo.tblGrading_SizingIssues.ParNo And dbo.tblGrading_SizingPacket.PktNo = dbo.tblGrading_SizingIssues.PktNo " & _
                      "WHERE (dbo.tblGrading_SizingPacket.Department = '" & cmbDept.Text & "') AND (dbo.tblGrading_SizingIssues.PktNo IS NULL) AND (dbo.tblParcel.Complete = 0) " & _
                      "ORDER BY dbo.tblGrading_SizingPacket.Department, dbo.tblGrading_SizingPacket.ParNo, dbo.tblGrading_SizingPacket.PktNo", AdoCN, 1, 1)
    ...
    Private Sub Save()
        ...
        For intRow = 0 To flxDetails.Rows.Count - 1
            If flxDetails.Item(5, intRow).Value = "1" Then
                rsComSql = New ADODB.Recordset
                rsComSql.Open("SELECT * FROM tblGrading_SizingIssues WHERE Department = '" & flxDetails.Item(0, intRow).Value & "' AND ParNo = '" & flxDetails.Item(1, intRow).Value & "' AND PktNo = '" & flxDetails.Item(2, intRow).Value & "' AND Sec = " & intSec & "", AdoCN, 1, 1)
                If rsComSql.RecordCount Then
