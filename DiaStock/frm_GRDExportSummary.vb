...
Public Class frm_GRDExportSummary
    Private Sub GetPackNo()
        rsComSql = New ADODB.Recordset
        rsComSql.Open("SELECT MAX(PackNo) AS MaxNo FROM tblGrading_PackingList", AdoCN, 1, 1)
    ...
    Private Sub Load_ExportParcels()
        ...
        rsComSql = New ADODB.Recordset
        rsComSql.Open("SELECT DISTINCT TOP (100) PERCENT dbo.tblGrading_SizingTypes.Department, dbo.tblGrading_SizingTypes.ParNo " & _
                      "FROM dbo.tblGrading_SizingTypes INNER JOIN dbo.tblGrading_Parcel ON LEFT(dbo.tblGrading_SizingTypes.ParNo, 6) = LEFT(dbo.tblGrading_Parcel.ParNo, 6) " & _
                      "WHERE (dbo.tblGrading_Parcel.Complete = 0) AND (dbo.tblGrading_SizingTypes.Ok = 0) AND (LEFT(dbo.tblGrading_SizingTypes.Department, 10) <> 'GradingPCU') " & _
                      "ORDER BY dbo.tblGrading_SizingTypes.ParNo", AdoCN, 1, 1)
        If rsComSql.RecordCount Then
            rsComSql.MoveFirst()
    ...
    Private Sub Load_ExportDetails(ByVal strParcel As String, ByVal strDepartment As String)
        ...
        dblRghIssCts = 0
        rsComSql = New ADODB.Recordset
        If strDepartment = "Lamour" Then
            rsComSql.Open("SELECT TOP (100) PERCENT dbo.tblGrading_SizingTypes.Department, dbo.tblGrading_SizingTypes.ReturnType, dbo.tblGrading_SizingTypes.ParNo, " & _
                                "dbo.tblGrading_SizingTypes.Pcs, dbo.tblGrading_SizingTypes.Cts, dbo.tblGrading_SizingList.Price, dbo.tblGrading_SizingTypes.SizeRange " & _
                          ...
                          "WHERE (dbo.tblGrading_SizingTypes.ParNo = '" & strParcel & "') AND (dbo.tblGrading_SizingTypes.Department = '" & strDepartment & "') AND (dbo.tblGrading_SizingTypes.OK = 0) " & _
                          "ORDER BY dbo.tblGrading_SizingTypes.ReturnType, dbo.tblGrading_SizingTypes.Cts", AdoCN, 1, 1)
        Else
            rsComSql.Open("SELECT TOP (100) PERCENT dbo.tblGrading_SizingTypes.Department, dbo.tblGrading_SizingTypes.ReturnType, dbo.tblGrading_SizingTypes.ParNo, " & _
                                "SUM(dbo.tblGrading_SizingTypes.Pcs) AS Pcs, ROUND(SUM(dbo.tblGrading_SizingTypes.Cts), 3) AS Cts, dbo.tblGrading_SizingList.PRICE, dbo.tblGrading_SizingTypes.SizeRange " & _
                          "FROM dbo.tblGrading_SizingTypes INNER JOIN dbo.tblGrading_SizingList ON dbo.tblGrading_SizingTypes.ReturnType = dbo.tblGrading_SizingList.NAME INNER JOIN " & _
                                "dbo.tblGrading_SizingPacket ON dbo.tblGrading_SizingTypes.Department = dbo.tblGrading_SizingPacket.Department AND " & _
                                "dbo.tblGrading_SizingTypes.ParNo = dbo.tblGrading_SizingPacket.ParNo And dbo.tblGrading_SizingTypes.PktNo = dbo.tblGrading_SizingPacket.PktNo " & _
                          "WHERE (dbo.tblGrading_SizingTypes.ParNo = '" & strParcel & "') AND (dbo.tblGrading_SizingTypes.Department = '" & strDepartment & "') AND (dbo.tblGrading_SizingTypes.OK = 0) " & _
                          "GROUP BY dbo.tblGrading_SizingTypes.Department, dbo.tblGrading_SizingTypes.ReturnType, dbo.tblGrading_SizingList.PRICE, dbo.tblGrading_SizingTypes.ParNo, dbo.tblGrading_SizingTypes.SizeRange " & _
                          "ORDER BY dbo.tblGrading_SizingTypes.ReturnType, dbo.tblGrading_SizingTypes.SizeRange", AdoCN, 1, 1)
        End If
        If rsComSql.RecordCount Then
            rsComSql.MoveFirst()
            While Not rsComSql.EOF
                ...
                rsComSql_1.Open("SELECT SUM(IssPcs) AS IssPcs, ROUND(SUM(IssCts), 3) AS IssCts " & _
                                ...
                                "WHERE (Export = 0) AND (Department = '" & rsComSql.Fields("Department").Value & "') AND " & _
                                    "(ParNo = '" & rsComSql.Fields("ParNo").Value & "') AND (Assortment = '" & rsComSql.Fields("ReturnType").Value & "') AND (SizeRange = '" & rsComSql.Fields("SizeRange").Value & "')", AdoCN, 1, 1)
                ...
                'If Mid(rsComSql.Fields("ReturnType").Value, 1, 2) = "AE" Then
                '    rsComSql_1 = New ADODB.Recordset
                '    rsComSql_1.Open("SELECT dbo.tblImport.ItemCost " & _
                '                    "FROM dbo.tblImport INNER JOIN dbo.tblParcel ON dbo.tblImport.SupParcelNo = dbo.tblParcel.OrigParcelNo " & _
                '                    "WHERE (dbo.tblParcel.GrpParNo = '" & rsComSql.Fields("ParNo").Value & "') AND (dbo.tblParcel.Depart = '" & rsComSql.Fields("Department").Value & "')", AdoCN, 1, 1)
                '    If rsComSql_1.RecordCount Then
                ...
                If rsComSql.Fields("Pcs").Value - intRghIssPcs > 0 Then
                    flxDetails.Rows.Add(rsComSql.Fields("ReturnType").Value,
                                        ...
                                        Format(dblPrice * (rsComSql.Fields("Cts").Value - dblRghIssCts), "#0.00"),
    ...
    Private Sub Save()
        ...
        rsComSql = New ADODB.Recordset
        rsComSql.Open("SELECT * FROM tblGrading_PackingList WHERE PackNo = " & Val(txtPackNo.Text) & "", AdoCN, 1, 1)
        If rsComSql.RecordCount Then
            PBResponse = MsgBox("Are you sure?", MsgBoxStyle.Question + MsgBoxStyle.YesNo, Me.Text)
            If PBResponse = MsgBoxResult.Yes Then
                For intRow = 0 To flxDetails.Rows.Count - 1
                    AdoCN.Execute("UPDATE tblGrading_PackingList SET ActPcs = " & CInt(flxDetails.Item(4, intRow).Value) & ",ActCts = " & CDbl(flxDetails.Item(5, intRow).Value) & " " & _
        ...
        Else
            For intRow = 0 To flxDetails.Rows.Count - 1
                If flxDetails.Item(11, intRow).Value = True Then
                    AdoCN.Execute("INSERT INTO tblGrading_PackingList(Department, PackNo, Assortment, ParNo, Pcs, Cts, ActPcs, ActCts, Price, OrderNo, RefNo, Side, SizeRange) " & _
                                  "VALUES('" & flxDetails.Item(9, intRow).Value & "'," & CDbl(txtPackNo.Text) & ",'" & flxDetails.Item(0, intRow).Value & "','" & flxDetails.Item(1, intRow).Value & "'," & CInt(flxDetails.Item(2, intRow).Value) & "," & _
                                        ...
                                        "" & CDbl(flxDetails.Item(7, intRow).Value) & ",'','','','" & flxDetails.Item(12, intRow).Value & "')")

                    AdoCN.Execute("UPDATE tblGrading_SizingTypes SET OK = 1 WHERE Department  = '" & flxDetails.Item(9, intRow).Value & "' AND ParNo = '" & flxDetails.Item(1, intRow).Value & "' AND ReturnType = '" & flxDetails.Item(0, intRow).Value & "' AND OK = 0")
                    AdoCN.Execute("UPDATE tblGrading_RghIssues SET Export = 1 WHERE Department  = '" & flxDetails.Item(9, intRow).Value & "' AND ParNo = '" & flxDetails.Item(1, intRow).Value & "' AND Assortment = '" & flxDetails.Item(0, intRow).Value & "' AND Export = 0")
                End If
            Next
