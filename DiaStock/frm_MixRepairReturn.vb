...
Public Class frm_MixRepairReturn
    ...
    Private Sub Load_Section()
        ...
        rsComSql.Open("SELECT * FROM tblMixSectionsRep WHERE SecCode > 1 ORDER BY SecCode", AdoCN, 1, 1)
    ...
    Private Sub Load_Section2()
        cmbSection2.Items.Clear()
        rsComSql = New ADODB.Recordset
        rsComSql.Open("SELECT * FROM tblMixSectionsRep ORDER BY SecCode", AdoCN, 1, 1)
        If rsComSql.RecordCount Then
            rsComSql.MoveFirst()
            While Not rsComSql.EOF
                cmbSection2.Items.Add(rsComSql.Fields("SecName").Value)
                rsComSql.MoveNext()
            End While
        End If
        rsComSql = Nothing
    ...
    Private Sub Load_Details()
        ...
        rsComSql.Open("SELECT TOP (100) PERCENT dbo.tblMixIssuesRep.ID, dbo.tblMixIssuesRep.ParNo, dbo.tblMixIssuesRep.PktNo, dbo.tblMixIssuesRep.Sec, dbo.tblMixIssuesRep.IssPcs, " & _
                      ...
                      "WHERE (dbo.tblMixReturnsRep.PktNo IS NULL) AND (dbo.tblMixIssuesRep.Sec = " & cmbSection.SelectedIndex + 2 & ") " & _
                      "ORDER BY dbo.tblMixIssuesRep.ParNo, dbo.tblMixIssuesRep.PktNo", AdoCN, 1, 1)
        If rsComSql.RecordCount Then
            rsComSql.MoveFirst()
            While Not rsComSql.EOF
                flxDetails.Rows.Add(cmbSection.Text,
    ...
    Private Sub Save()
        ...
        If chkTrf.Checked = True Then
            ...
            If cmbSection2.Text = "" Then MsgBox("Invalid Next Section", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, Me.Text) : Exit Sub
            If cmbSection.Text = cmbSection2.Text Then MsgBox("Invalid Next Section", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, Me.Text) : Exit Sub
        End If
        ...
        For intRow = 0 To flxDetails.Rows.Count - 1
            If flxDetails.Item(4, intRow).Value = True Then
                rsComSql = New ADODB.Recordset
                rsComSql.Open("SELECT * FROM tblMixReturnsRep WHERE ParNo = '" & flxDetails.Item(1, intRow).Value & "' AND PktNo = '" & flxDetails.Item(2, intRow).Value & "' AND IssueID = '" & CDbl(flxDetails.Item(5, intRow).Value) & "' AND Sec = " & cmbSection.SelectedIndex + 2 & "", AdoCN, 1, 1)
                If rsComSql.RecordCount Then
        ...
        For intRow = 0 To flxDetails.Rows.Count - 1
            If flxDetails.Item(4, intRow).Value = True Then
                ...
                If rsComSql.RecordCount = 0 Then
                    blnSave = True
                    rsComSql = New ADODB.Recordset
                    rsComSql.Open("SELECT * FROM tblMixReturnsRep WHERE ParNo = '" & flxDetails.Item(1, intRow).Value & "' AND PktNo = '" & flxDetails.Item(2, intRow).Value & "' AND Sec = " & cmbSection.SelectedIndex + 2 & "", AdoCN, 1, 1)
                    ...
                    AdoCN.Execute("INSERT INTO tblMixReturnsRep(IssueID,ParNo,PktNo,Sec,RetPcs,EmpNo,RetDate,RetTime,EmpNo2,IndexNo) " & _
                                  "VALUES('" & CDbl(flxDetails.Item(5, intRow).Value) & "','" & flxDetails.Item(1, intRow).Value & "','" & flxDetails.Item(2, intRow).Value & "'," & cmbSection.SelectedIndex + 2 & "," & CInt(flxDetails.Item(3, intRow).Value) & ",'" & flxDetails.Item(6, intRow).Value & "'," & _
                                    "'" & Format(Date.Now, "MM/dd/yyyy") & "','" & Format(Date.Now, "HH:mm") & "','" & PBUser_EmpNo & "','" & intIndex & "')")

                    If chkTrf.Checked = True Then
                        rsComSql = New ADODB.Recordset
                        rsComSql.Open("SELECT * FROM tblMixIssuesRep WHERE ParNo = '" & flxDetails.Item(1, intRow).Value & "' AND PktNo = '" & flxDetails.Item(2, intRow).Value & "' AND Sec = " & cmbSection.SelectedIndex + 1 & "", AdoCN, 1, 1)
                        intIndex = rsComSql.RecordCount + 1
                        rsComSql = Nothing

                        AdoCN.Execute("INSERT INTO tblMixIssuesRep(ParNo,PktNo,Sec,IssPcs,EmpNo,IssDate,IssTime,EmpNo2,IndexNo) " & _
                                      "VALUES('" & flxDetails.Item(1, intRow).Value & "','" & flxDetails.Item(2, intRow).Value & "'," & cmbSection.SelectedIndex + 1 & "," & CInt(flxDetails.Item(3, intRow).Value) & ",'" & Mid(Trim(txtEmp.Text), 1, 6) & "'," & _
                                        "'" & Format(Date.Now, "MM/dd/yyyy") & "','" & Format(Date.Now, "HH:mm") & "','" & PBUser_EmpNo & "','" & intIndex & "')")
                    End If
                End If
                rsComSql = Nothing
    ...
    Private Sub frm_MixRepairReturn_Load(sender As Object, e As EventArgs) Handles MyBase.Load
