...
Public Class frm_ExpSizingTransfer
    Private Sub Load_SizingDetails()
        ...
        If txtParcel.Text = "" Then
            'rsComSql.Open("SELECT * FROM tblExpSizingTypes WHERE OK = 0 AND Sec = 1 " & _
            ...
            Exit Sub
        Else
            rsComSql.Open("SELECT * FROM tblExpSizingTypes WHERE ParNo = '" & txtParcel.Text & "' AND OK = 0 AND Sec = 1 " & _
                          "ORDER BY Department, ParNo, PktNo, ReturnType", AdoCN, 1, 1)
        End If
        If rsComSql.RecordCount Then
            ...
            While Not rsComSql.EOF
                ...
                Else
                    ...
                    rsComSql_2.Open("SELECT * FROM tblDCLPermanents WHERE ItemName = '" & rsComSql.Fields("ReturnType").Value & "'", AdoCN, 1, 1)
                ...
                If rsComSql_1.RecordCount Then
                    ...
                    dblDiaPrice = rsComSql_1.Fields("ActItemCost").Value
                Else
                    rsComSql_2 = New ADODB.Recordset
                    ...
                    If rsComSql_2.RecordCount Then
                        ...
                        dblDiaPrice = rsComSql_2.Fields("ActItemCost").Value
                    Else
                ...
                flxDetails.Rows.Add(rsComSql.Fields("Department").Value,
                                    ...
                                    Format(rsComSql.Fields("Cts").Value - dblIssCts, "#0.000"),
                                    ...
                                    Format(rsComSql.Fields("EstCts").Value - dblIssCts, "#0.000"),
                                    dblDiaPrice)

                dblListValue = dblListValue + (dblBasePrice * (rsComSql.Fields("Cts").Value - dblIssCts))
                dblInvValue = dblInvValue + (dblInvPrice * (rsComSql.Fields("Cts").Value - dblIssCts))
                dblDiaValue = dblDiaValue + (dblDiaPrice * (rsComSql.Fields("Cts").Value - dblIssCts))
                rsComSql.MoveNext()
            ...
            End While
        Else
            MsgBox("No Records to Transfer", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, Me.Text)
        End If
        rsComSql = Nothing

        txtInvValue.Text = Math.Round(dblInvValue, 2)
        txtListValue.Text = Math.Round(dblListValue, 2)
        txtDiaValue.Text = Math.Round(dblDiaValue, 2)

        If CDbl(txtListValue.Text) + 5 < CDbl(txtInvValue.Text) And strDBName = "DiaSales" Then
            Process()
        Else
            If CDbl(txtListValue.Text) + 5 < CDbl(txtInvValue.Text) And txtDepartment.Text = "KIT Box" Then
                Process()
            End If
        End If
        ...
        If chkSelect.Checked = True Then
            For intRow = 0 To flxDetails.RowCount - 1
                If optRough.Checked = True Then
                    If Mid(flxDetails.Item(4, intRow).Value, 1, 2) = "AR" Or Mid(flxDetails.Item(4, intRow).Value, 7, 1) = "R" Then
                        flxDetails.Item(11, intRow).Value = True
                    End If
                End If
                If optRoughS.Checked = True Then
                    If Mid(flxDetails.Item(4, intRow).Value, 1, 2) = "SR" Or Mid(flxDetails.Item(4, intRow).Value, 1, 2) = "SS" Or Mid(flxDetails.Item(4, intRow).Value, 7, 1) = "S" Then
                        flxDetails.Item(11, intRow).Value = True
                    End If
                End If
        ...
        If PBResponse = MsgBoxResult.Yes Then
            For intRow = 0 To flxDetails.Rows.Count - 1
                If flxDetails.Item(11, intRow).Value = True Then
                    ...
                    If Len(flxDetails.Item(12, intRow).Value) = 0 Then
                        MsgBox("Invalid Category - " & flxDetails.Item(4, intRow).Value, MsgBoxStyle.Information + MsgBoxStyle.OkOnly, Me.Text)
                        Exit Sub
                    End If
                End If
            Next
            ...
            For intRow = 0 To flxDetails.Rows.Count - 1
                If flxDetails.Item(11, intRow).Value = True Then
                    ...
                    If strDBName = "DiaStock" Or strDBName = "DiaShare" Then
                        intStatus = 4
                    Else
                        If flxDetails.Item(12, intRow).Value = "Purchased" Then
                            intStatus = 2
                        Else
                            intStatus = 5
                        End If
                    ...
                    If CDbl(txtListValue.Text) + 5 < CDbl(txtInvValue.Text) And strDBName = "DiaSales" Then
                        AdoCN.Execute("UPDATE tblExpSizingTypes SET OK = " & intStatus & ", BasePrice = '" & CDbl(flxDetails.Item(9, intRow).Value) & "', DiaCost = '" & CDbl(flxDetails.Item(15, intRow).Value) & "' " & _
                                      "WHERE ID = " & CDbl(flxDetails.Item(13, intRow).Value) & "")
                    Else
                        AdoCN.Execute("UPDATE tblExpSizingTypes SET OK = " & intStatus & ", BasePrice = '" & CDbl(flxDetails.Item(7, intRow).Value) & "', DiaCost = '" & CDbl(flxDetails.Item(15, intRow).Value) & "' " & _
                                      "WHERE ID = " & CDbl(flxDetails.Item(13, intRow).Value) & "")
                    End If
    ...
    Private Sub Process()
        ...
        If flxDetails.Rows.Count >= 1 Then
            ...
            dblTotBase = 0
            For intRow = 0 To flxDetails.Rows.Count - 1
                dblTotBase = dblTotBase + (CDbl(flxDetails.Item(6, intRow).Value) * CDbl(flxDetails.Item(7, intRow).Value))
            Next
            ...
            If dblTotBase > 0 Then
                For intRow = 0 To flxDetails.Rows.Count - 1
                    dblAdjValue = (dblTotInv / dblTotBase) * (CDbl(flxDetails.Item(6, intRow).Value) * CDbl(flxDetails.Item(7, intRow).Value))
                    dblAdjCost = dblAdjValue / CDbl(flxDetails.Item(6, intRow).Value)
                    flxDetails.Item(9, intRow).Value = Format(Math.Round(dblAdjCost, 2), "#0.00")
                Next
            End If
    ...
    Private Sub ProcessDiaCost()
        ...
        If flxDetails.Rows.Count >= 1 Then
            ...
            dblTotInv = CDbl(txtDiaValue.Text)

            If dblTotBase > 0 Then
                For intRow = 0 To flxDetails.Rows.Count - 1
                    dblAdjValue = (dblTotInv / dblTotBase) * (CDbl(flxDetails.Item(6, intRow).Value) * CDbl(flxDetails.Item(7, intRow).Value))
                    dblAdjCost = dblAdjValue / CDbl(flxDetails.Item(6, intRow).Value)
                    flxDetails.Item(15, intRow).Value = Format(Math.Round(dblAdjCost, 2), "#0.00")
                Next
            End If
