...
Public Class frm_MixConfirmOrders
    ...
    Private Sub Load_MixFinishOrders()
        ...
        Dim dblAskingCost As Double
        Dim dblAskingValue As Double
        ...
        If txtOrder.Text = "" And cmbClient.Text = "" Then
            rsComSql.Open("SELECT TOP (100) PERCENT dbo.tblMixFinishOrders.OrderNo, dbo.tblMixFinishOrders.Reference, dbo.tblMixFinishOrders.Side, dbo.tblMixFinishOrders.ParNo, " & _
                            ...
                            "dbo.tblMixFinishOrders.IssuePcs, dbo.tblAssortList.CurrentCost, dbo.tblAssortList.AvgCost, dbo.tblOrders.Niruref " & _
                          "FROM dbo.tblMixFinishOrders INNER JOIN dbo.tblAssortList ON dbo.tblMixFinishOrders.Assortment = dbo.tblAssortList.Assortment INNER JOIN dbo.tblOrders ON dbo.tblMixFinishOrders.OrderNo = dbo.tblOrders.OrderNo " & _
        ...
        ElseIf cmbClient.Text <> "" And txtOrder.Text = "" Then
            rsComSql.Open("SELECT TOP (100) PERCENT dbo.tblMixFinishOrders.OrderNo, dbo.tblMixFinishOrders.Reference, dbo.tblMixFinishOrders.Side, dbo.tblMixFinishOrders.ParNo, " & _
                            ...
                            "dbo.tblMixFinishOrders.IssuePcs, dbo.tblAssortList.CurrentCost, dbo.tblAssortList.AvgCost, dbo.tblOrders.Niruref " & _
                          "FROM dbo.tblMixFinishOrders INNER JOIN dbo.tblAssortList ON dbo.tblMixFinishOrders.Assortment = dbo.tblAssortList.Assortment INNER JOIN dbo.tblOrders ON dbo.tblMixFinishOrders.OrderNo = dbo.tblOrders.OrderNo " & _
                          "WHERE (dbo.tblMixFinishOrders.Status LIKE 'A') AND (dbo.tblMixFinishOrders.Export = 1) AND (dbo.tblOrders.Niruref = '" & cmbClient.Text & "') " & _
                          "ORDER BY dbo.tblMixFinishOrders.OrderNo, dbo.tblMixFinishOrders.PacketNo", AdoCN, 1, 1)
        ...
        If rsComSql.RecordCount Then
            ...
            mStrSQL = "SELECT MAX(RecordNo) AS RecordNo FROM dbo.tblMixExportOrders"
            ...
            While Not rsComSql.EOF
                ...
                rsComSql_1.Open("SELECT MaxType, MaxCost, AskingPrice " & _
                                "FROM dbo.tblOrdersDtls " & _
                                "WHERE (OrderNo = '" & rsComSql.Fields("OrderNo").Value & "') AND (RefNo = '" & Replace(rsComSql.Fields("Reference").Value, "'", "''") & "') AND (Side = '" & rsComSql.Fields("Side").Value & "')", AdoCN, 1, 1)
                If rsComSql_1.RecordCount Then
                    strMaxType = rsComSql_1.Fields("MaxType").Value
                    dblMaxCost = rsComSql_1.Fields("MaxCost").Value
                    dblAskingCost = rsComSql_1.Fields("AskingPrice").Value
                End If
                rsComSql_1 = Nothing

                If strMaxType = "P" Then
                    dblMaxValue = dblMaxCost * rsComSql.Fields("FinishedPcs").Value
                    dblAskingValue = dblAskingCost * rsComSql.Fields("FinishedPcs").Value
                Else
                    dblMaxValue = dblMaxCost * rsComSql.Fields("FinishedCts").Value
                    dblAskingValue = dblAskingCost * rsComSql.Fields("FinishedCts").Value
                End If
                dblMaxValue = Math.Round(dblMaxValue, 2)
                dblAskingValue = Math.Round(dblAskingValue, 2)
    ...
    Private Sub Load_SavedData()
        flxDetails.Rows.Clear()
        rsComSql = New ADODB.Recordset
        mStrSQL = "SELECT * FROM dbo.tblMixExportOrders WHERE Status LIKE 'A' AND OrderNo <> 'Returns' ORDER BY OrderNo"
        ...
        If rsComSql.RecordCount Then
            While Not rsComSql.EOF
                flxDetails.Rows.Add(rsComSql.Fields("OrderNo").Value,
                                    ...
                                    rsComSql.Fields("ParNo").Value,
                                    rsComSql.Fields("Assortment").Value,
                                    Math.Round(rsComSql.Fields("AssPrice").Value, 2),
                                    rsComSql.Fields("PacketNo").Value,
                                    rsComSql.Fields("FinishedPcs").Value,
    ...
    Private Sub SaveData()
        ...
        If optNew.Checked = True Then
            ...
            dblRecord = 0
            For iRow = 0 To flxDetails.Rows.Count - 1
                If flxDetails.Item(23, iRow).Value = True Or flxDetails.Item(23, iRow).Value = 1 Then
                    AdoCN.Execute("INSERT INTO dbo.tblMixExportOrders(OrderNo,Subject,Reference,Side,ParNo,Assortment,AssPrice,PacketNo,FinishedPcs,FinishedCts,PacketPcs," & _
                                "PacketCts,IssueCts,RateCode,OrigAssort,Export,Status,AuditNo,RecordNo,InID,NLineNo,ModifyBy,DoneBy,GrPcs,RetDate,AvgCost,IssuePcs,PackingListNo,ClientID) " & _
                              "VALUES('" & flxDetails.Item(0, iRow).Value & "','" & flxDetails.Item(1, iRow).Value & "','" & Replace(flxDetails.Item(2, iRow).Value, "'", "''") & "'," & _
                                ...
                                "1,'A',0," & CDbl(flxDetails.Item(14, iRow).Value) & ",0,'" & flxDetails.Item(16, iRow).Value & "','" & PBUser_ID & "','" & PBUser_ID & "'," & _
                                "" & CDbl(flxDetails.Item(17, iRow).Value) & ",'" & Format(Date.Now, "MM/dd/yyyy") & "'," & _
                                "" & CDbl(flxDetails.Item(18, iRow).Value) & "," & CDbl(flxDetails.Item(19, iRow).Value) & "," & CDbl(txtPack.Text) & ",'" & flxDetails.Item(20, iRow).Value & "')")

                    AdoCN.Execute("UPDATE dbo.tblMixPacketDetails " & _
                                  "SET Ok = 1 " & _
                                  "WHERE ParNo = '" & flxDetails.Item(0, iRow).Value & "' AND PktNo = '" & flxDetails.Item(7, iRow).Value & "'")

                    AdoCN.Execute("UPDATE dbo.tblMixReturns " & _
                                  "SET Status = 1 " & _
                                  "WHERE ParNo = '" & flxDetails.Item(0, iRow).Value & "' AND PktNo = '" & flxDetails.Item(7, iRow).Value & "' AND Sec = 18 AND Status = 2")
                End If
    ...
    Private Function CalTotalPcs(ByVal flxSample As System.Windows.Forms.DataGridView, ByVal intCalRow As Integer) As Integer
        ...
        For intRow = 0 To flxSample.Rows.Count - 1
            If flxSample.Rows(intRow).Cells(23).EditedFormattedValue = True Then
    ...
    Private Sub Delete()
        ...
        rsComSql = New ADODB.Recordset
        rsComSql.Open("SELECT * FROM tblMixExportOrders WHERE PackingListNo = " & CDbl(txtPack.Text) & " AND Status = 'E'", AdoCN, 1, 1)
        If rsComSql.RecordCount = 0 Then
            PBResponse = MsgBox("Are you sure to Delete?", MsgBoxStyle.Question + MsgBoxStyle.YesNo, Me.Text)
            If PBResponse = MsgBoxResult.Yes Then
                AdoCN.Execute("DELETE FROM tblMixExportOrders WHERE PackingListNo = " & CDbl(txtPack.Text) & "")

                MsgBox("Confirm Orders Deleted", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, Me.Text)
            End If
        Else
            MsgBox("Cannot Delete. Export Verfication Already Seved.", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, Me.Text)
        End If
        rsComSql = Nothing
    End Sub
    ...
    Private Sub chkSelect_CheckedChanged(sender As Object) Handles chkSelect.CheckedChanged
        ...
        If chkSelect.Checked = True Then
            For intRow = 0 To flxDetails.RowCount - 1
                flxDetails.Item(23, intRow).Value = True
            Next
        Else
            For intRow = 0 To flxDetails.RowCount - 1
                flxDetails.Item(23, intRow).Value = False
            Next
        End If
        txtPcs.Text = CalTotalPcs(flxDetails, 8)
        txtCts.Text = CalTotalCts(flxDetails, 9)
