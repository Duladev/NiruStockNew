...
Public Class frm_MixPacketGrp
    ...
    Private Sub Load_Packets()
        ...
        rsComSql.Open("SELECT TOP (100) PERCENT dbo.tblMixPacket.PktOrdNo, dbo.tblMixPacket.PktNo, dbo.tblMixPacket.Grp, dbo.tblMixPacket.PktFlow, dbo.tblMixPacket.PktPcs, dbo.tblMixPacket.PktCts " & _
                      "FROM dbo.tblMixPacket LEFT OUTER JOIN dbo.VW_MixIssues14 ON dbo.tblMixPacket.PktOrdNo = dbo.VW_MixIssues14.ParNo AND dbo.tblMixPacket.PktNo = dbo.VW_MixIssues14.PktNo " & _
                      "WHERE (dbo.tblMixPacket.PktOrdNo = '" & txtOrderNo.Text & "') AND (dbo.VW_MixIssues14.PktNo IS NULL) AND (dbo.tblMixPacket.Ok = 1) AND (dbo.tblMixPacket.Accept = 1) " & _
                      "ORDER BY dbo.tblMixPacket.PktNo", AdoCN, 1, 1)
        If rsComSql.RecordCount Then
            rsComSql.MoveFirst()
    ...
    Private Sub Load_Packets2()
        If txtFrom.Text <> "" And txtTo.Text <> "" Then
            If Len(txtFrom.Text) = 6 And Len(txtTo.Text) = 6 Then
        ...
        rsComSql.Open("SELECT TOP (100) PERCENT dbo.tblMixPacket.PktOrdNo, dbo.tblMixPacket.PktNo, dbo.tblMixPacket.Grp, dbo.tblMixPacket.PktFlow, dbo.tblMixPacket.PktPcs, dbo.tblMixPacket.PktCts " & _
                      "FROM dbo.tblMixPacket LEFT OUTER JOIN dbo.VW_MixIssues14 ON dbo.tblMixPacket.PktOrdNo = dbo.VW_MixIssues14.ParNo AND dbo.tblMixPacket.PktNo = dbo.VW_MixIssues14.PktNo " & _
                      "WHERE (dbo.tblMixPacket.PktOrdNo >= '" & txtFrom.Text & "') AND (dbo.tblMixPacket.PktOrdNo <= '" & txtTo.Text & "') AND (dbo.VW_MixIssues14.PktNo IS NULL) AND (dbo.tblMixPacket.Ok = 1) AND (dbo.tblMixPacket.Accept = 1) " & _
                      "ORDER BY dbo.tblMixPacket.PktNo", AdoCN, 1, 1)
        If rsComSql.RecordCount Then
            rsComSql.MoveFirst()
            While Not rsComSql.EOF
        ...
        If PBResponse  = MsgBoxResult.Yes Then
            If txtOrderNo.Text <> "" And txtGroup.Text <> "" Then
                For intRow = 0 To flxDetails.Rows.Count - 1
                    If flxDetails.Item(6, intRow).Value = True Then
                        rsComSql = New ADODB.Recordset
                        rsComSql.Open("SELECT NewGrp FROM tblMixPacket WHERE PktOrdNo = '" & flxDetails.Item(0, intRow).Value & "' AND PktNo = '" & flxDetails.Item(1, intRow).Value & "'", AdoCN, 1, 1)
                        If rsComSql.RecordCount Then
                            If rsComSql.Fields("NewGrp").Value <> "D" Then
                                AdoCN.Execute("UPDATE tblMixPacket SET Grp = '" & UCase(txtGroup.Text) & "',NewGrp = '" & UCase(txtGroup.Text) & "' " & _
                                              "WHERE PktOrdNo = '" & flxDetails.Item(0, intRow).Value & "' AND " & _
                                                    "PktNo = '" & flxDetails.Item(1, intRow).Value & "'")
                            Else
                                AdoCN.Execute("UPDATE tblMixPacket SET Grp = '" & UCase(txtGroup.Text) & "' " & _
                                              "WHERE PktOrdNo = '" & flxDetails.Item(0, intRow).Value & "' AND " & _
                                                    "PktNo = '" & flxDetails.Item(1, intRow).Value & "'")
                            End If
                        End If
