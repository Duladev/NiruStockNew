...
Public Class frm_MixPacketAccept

    Private Sub Load_Packets()

        flxDetails.Rows.Clear()
        rsComSql = New ADODB.Recordset
        rsComSql.Open("SELECT * FROM tblMixPacket WHERE PktOrdNo = '" & txtOrderNo.Text & "' AND Ok = 1 AND Accept = 0 ORDER BY PktNo", AdoCN, 1, 1)
            ...
            If txtOrderNo.Text <> "" Then
                For intRow = 0 To flxDetails.Rows.Count - 1
                    If flxDetails.Item(6, intRow).Value = True Then
                        AdoCN.Execute("UPDATE tblMixPacket SET Accept = 1,AcceptDate = '" & Format(Date.Now, "MM/dd/yyyy") & "', AcceptTime = '" & Format(Date.Now, "HH:mm") & "', AcceptBy = '" & PBUser_EmpNo & "' " & _
    ...
    Private Sub frm_MixPacketAccept_Load(sender As Object, e As EventArgs) Handles MyBase.Load
